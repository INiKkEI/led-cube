...
| IT-03 | Add eight 220 Ω resistors and eight LEDs to that register's outputs, cathodes to ground. | Each LED lights when its bit is set. About 6 mA per LED, about 48 mA total into the register. | |
...
| IT-09 | Fit only the jack and the decoupling capacitors, then apply power. | 5.0 V ±5 % at every IC supply pad. Nothing warm to the touch after a minute. | |
...
| IT-11 | Fit the eight layer MOSFETs and select each layer in turn. | Each cathode net pulls below 50 mV when selected, and floats when not. | |
...
| Test | How to test | Pass if | Result |
|---|---|---|:--:|
| IT-13 | Attach the bottom layer only, and light all 64 of its LEDs. | All 64 light. Layer current about 384 mA. Brightest and dimmest within ±20 % of each other. | |
