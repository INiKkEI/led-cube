...
- target instantaneous LED current when on: 6 mA

### Why 6 mA was chosen
The first revision is based on the selected 74HCT595 shift-register chain. In this project, each 74HCT595 output is treated as a 6 mA maximum practical output, so the power budget must use that same current basis.

This value is a planning assumption tied to the currently selected driver architecture, not a final measured operating point.
...
`I_LED_avg_total ≈ 384 mA`

So for planning purposes:

- instantaneous LED current, worst case: 384 mA
- average LED supply current for a fully filled cube: about 384 mA
...
`I_total_estimated = 384 mA + 200 mA = 584 mA`

So the first-pass total operating estimate is:

`I_total_estimated ≈ 0.58 A`
...
### 5 V input
The 5 V input path must support well over 0.5 A operating current with margin. This means the power input should be treated as a real power path, not a light logic-only connection.
...
The cube uses low-side layer switching. Each layer switch must handle the full active-layer current.

Minimum planning requirement:

`layer switch current > 0.384 A`

In practice, the chosen transistor should have clear margin above this number.

The gate or base control path must also switch the layer cleanly enough to avoid:
- ghosting
- incomplete turn-on
- unnecessary heating

## Column-driving implications
The column path must support the selected per-LED current of 6 mA for every lit LED in the active layer.
...
- driver or output-stage voltage drop
...
### Layer transistors
The low-side transistors carry the full active-layer current when enabled. Their conduction loss depends on the chosen device and operating point.

Exact dissipation depends on the chosen transistor and is not yet frozen, but the current budget already shows that these devices must not be selected casually.
...
| LEDs per layer | 64 |
| Multiplex ratio | 1:8 |
| Instantaneous current per lit LED | 6 mA |
| Worst-case active-layer current | 384 mA |
| Estimated ESP32 budget from 5 V | 150 mA |
| Estimated driver/support logic budget | 50 mA |
| Estimated total current | 584 mA |
...
- design around 6 mA instantaneous current per lit LED
- assume 384 mA worst-case active-layer current
- reserve about 200 mA for ESP32 and support logic
- plan for about 584 mA total estimated load
- use a 5 V, 2 A supply target for comfortable margin

These numbers are sufficient to continue with:
- component selection
- layer transistor selection
